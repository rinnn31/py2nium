...
import io.py2nium.server.model.api.gestures.ElementFlingModel;

public class FlingCommand extends CommandHandler {
    public FlingCommand(String uri, HttpMethod method) {
        super(uri, method);
    }
    ...
    protected Py2niumResponse handle(HttpReq request) {
        ...
        ElementFlingModel model = toModelAndValidate(request.getBody(), ElementFlingModel.class);

        AndroidElement element = AndroidDevice.getInstance().getElementsCache().getElement(elementId);
        boolean ret = element.fling(model.getDirection(), model.speed);
