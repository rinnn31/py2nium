...
import io.py2nium.server.model.api.KeysModel;

public class SendKeysCommand extends CommandHandler {
    public SendKeysCommand(String uri, HttpMethod... methods) {
        super(uri, methods);
    }

    @Override
    protected Py2niumResponse handle(HttpReq request) {
        KeysModel model = toModelAndValidate(request.getBody(), KeysModel.class);

        InteractionController ic = AndroidDevice.getInstance().getInteractionController();
        int[] keyCodes = new int[model.keyCodes.length];
        for (int i = 0; i < model.keyCodes.length; i++) {
            keyCodes[i] = model.keyCodes[i];
        }

        int metaState = model.metaState == null ? 0 : model.metaState;
        boolean ret = ic.sendKeys(keyCodes, metaState);
