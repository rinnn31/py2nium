...
public class MatchImageCommand extends CommandHandler {
    public MatchImageCommand(String uri, HttpMethod... methods) {
        super(uri, methods);
    }

    @Override
    protected Py2niumResponse handle(HttpReq request) {
        MatchImageModel model = toModelAndValidate(request.getBody(), MatchImageModel.class);
        return new Py2niumResponse(Py2niumResponse.SUCCESS, ImageLocator.matchImage(model), null);
    }
}
