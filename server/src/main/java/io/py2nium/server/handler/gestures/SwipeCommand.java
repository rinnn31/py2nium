...
import io.py2nium.server.model.api.gestures.SwipeModel;

public class SwipeCommand extends CommandHandler {
    public SwipeCommand(String uri, HttpMethod method) {
        super(uri, method);
    }

    @Override
    protected Py2niumResponse handle(HttpReq request) {
        SwipeModel model = toModelAndValidate(request.getBody(), SwipeModel.class);
        ...
        if(Boolean.FALSE.equals(model.isDragMode)) {
            gestureController.swipe(model.getStartPoint(), model.getEndPoint(), model.speed);
        } else {
            gestureController.drag(model.getStartPoint(), model.getEndPoint(), model.speed);
        }
