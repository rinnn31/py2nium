...
public class GetElementParentCommand extends CommandHandler {
    public GetElementParentCommand(String uri, HttpMethod... methods) {
        super(uri, methods);
    }
    ...
    protected Py2niumResponse handle(HttpReq request) {
        ...
        AndroidElement parent = element.getParent();
        if(parent != null) {
            String cachedElementId = AndroidDevice.getInstance().getElementsCache().putElement(parent);
            return new Py2niumResponse(Py2niumResponse.SUCCESS, new ElementModel(cachedElementId), null);
        }
