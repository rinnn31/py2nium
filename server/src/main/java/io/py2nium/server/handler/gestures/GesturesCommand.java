...
import io.py2nium.server.model.api.gestures.GestureModel;

public class GesturesCommand extends CommandHandler {
    public GesturesCommand(String uri, HttpMethod method) {
        super(uri, method);
    }

    @Override
    protected Py2niumResponse handle(HttpReq request) {
        GestureModel[] models = toModelAndValidate(request.getBody(), GestureModel[].class);

        InteractionController ic = AndroidDevice.getInstance().getInteractionController();
        for (GestureModel model : models) {
            switch (model.action) {
                case GestureModel.ACTION_TOUCH_UP:
                    ic.touchUp(model.x, model.y);
                    break;
                case GestureModel.ACTION_TOUCH_DOWN:
                    ic.touchDown(model.x, model.y);
                    break;
                case GestureModel.ACTION_TOUCH_MOVE:
                    ic.touchMove(model.x, model.y);
                    break;
                case GestureModel.ACTION_WAIT:
                    ic.wait(model.duration);
                    break;
            }
        }
