...
public class ClearClipboardCommand extends CommandHandler {
    public ClearClipboardCommand(String uri, HttpMethod... methods) {
        super(uri, methods);
    }
    @Override
    protected Py2niumResponse handle(HttpReq request) {
        InstrumentationRegistry.getInstrumentation().runOnMainSync(WrappedClipboardManager::clearClipboard);
