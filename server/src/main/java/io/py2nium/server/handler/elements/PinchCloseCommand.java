...
import io.py2nium.server.model.api.gestures.ElementPinchModel;

public class PinchCloseCommand extends CommandHandler {
    public PinchCloseCommand(String uri, HttpMethod... methods) {
        super(uri, methods);
    }
    ...
    protected Py2niumResponse handle(HttpReq request) {
        ...
        ElementPinchModel model = toModelAndValidate(request.getBody(), ElementPinchModel.class);

        AndroidElement element = AndroidDevice.getInstance().getElementsCache().getElement(elementId);
        element.pinchClose(model.percent, model.speed);
