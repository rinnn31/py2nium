...
import io.py2nium.server.utils.DeviceUtils;

public class GetDeviceInfoCommand extends CommandHandler {
    public GetDeviceInfoCommand(String uri, HttpMethod... methods) {
        super(uri, methods);
    }

    @Override
    protected Py2niumResponse handle(HttpReq request) {
        DeviceInfoModel model = DeviceUtils.getDeviceInfo();
        return new Py2niumResponse(Py2niumResponse.SUCCESS, model, null);
    }
}
