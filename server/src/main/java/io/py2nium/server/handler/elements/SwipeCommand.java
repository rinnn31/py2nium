...
import io.py2nium.server.model.api.gestures.ElementSwipeModel;
...
public class SwipeCommand extends CommandHandler {
    ...
    protected Py2niumResponse handle(HttpReq request) {
        ...
        ElementSwipeModel model = toModelAndValidate(request.getBody(), ElementSwipeModel.class);

        AndroidElement element = AndroidDevice.getInstance().getElementsCache().getElement(elementId);
        element.swipe(model.getDirection(), model.percent, model.speed);
